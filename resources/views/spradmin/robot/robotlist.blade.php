...
<br>
               ...
               <div class="sparkline13-list">
                  <div class="sparkline13-hd">
                     <div class="main-sparkline13-hd">
                        <h1> Robot  <span class="table-project-n">Data</span> Table</h1>
                     </div>
                  </div>
                    <div class="text-right">
                  <a href="javascript:void(0)" onclick="return addNewRobot()" class="btn btn-success btn-sm"><i class="fa fa-plus" aria-hidden="true"></i>Add New Robot </a>
                 </div>
                        ...
                        <div class="bootstrap-table">
                           <div class="fixed-table-toolbar">
                              ...
                              <table id="table" data-toggle="table" data-pagination="true" data-search="true" data-show-columns="true" data-show-pagination-switch="true" data-show-refresh="true" data-key-events="true" data-show-toggle="true" data-resizable="true" data-cookie="true" data-cookie-id-table="saveId" data-show-export="true" data-click-to-select="true" data-toolbar="#toolbar" class="table table-hover JColResizer">
                                 ...
                                 <tbody id="loadrobotDataTableSection">
 ...
 <!--  Jquery Model popup  -->
<div id="RobotupdateformWith" class="modal fade" tabindex="-1">
   <div class="modal-dialog">
      <div class="modal-content">
         <div class="modal-header" style="background-color: #FF00FF !important; color: white; text-align: bold;">
            <h5 class="modal-title">Robot Update Form</h5>
            <button type="button" class="close" data-dismiss="modal">&times;</button>
         </div>
         <div class="modal-body">
             <form name="update_form" id="update_form"  onsubmit="return robotUpdate()" > 
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header" style="background-color: #C71585; color: white;">
        <h5 class="modal-title">Add New Robot</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
       <form name="addnewmembesForm" id="addnewmembesForm" onsubmit="return addnewRobotFor()">
        @csrf 
        <input type="hidden" name="roll_id" id="roll_id" value="5">
       <div class="row">
           <div class="col-md-6">
              <div class="form-group">
...
{ 
 ...
 $.ajax({
          url: "{{ url('spr-admin/robotlist') }}",
          type: "get",
          cache: false,
      success: function(data){
           $('#loadrobotDataTableSection').html(data);
        }
    });

}

  function robotStatusChange(id,status)
     {  
    swal({
          title: "Are you sure?",
  ...
  function roboteditform(id)
   ...
   .then((willDelete) => {
      if (willDelete) {
      $.ajax({
             url: '{{url("spr-admin/getSingleRobot")}}',
             type: 'post',
             dataType: 'json',
             data: {"_token": "{{ csrf_token() }}", id : id},
       success:function(response){
                ...
                $('#RobotupdateformWith .modal-body').html();
                $('#RobotupdateformWith').modal();
               }
           });
        return false;
    } 
  ...
  var updateData =  $( "#update_form" ).serialize();
      $.ajax({
             type: 'POST',
             url: '{{url("spr-admin/robotUpDate")}}',
             data: updateData,
       success:function(response){
              $('#RobotupdateformWith').modal('hide');
               getallRobot();
               if (response.status == 'success') {
                swal({
                   icon: "success",
    ...
    function addNewRobot(){ 
     swal({
            title: "Are you sure?",
            text: "You want to Add New Robot!",
            icon: "warning",
            buttons: true,
            successMode: true,
   })
